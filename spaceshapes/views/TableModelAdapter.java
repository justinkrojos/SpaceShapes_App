...
/**
 * Adapter class that adapts Shape/CarrierShape to the TableModel target 
 * interface. An instance of TableModelAdapter thus allows a Shape/CarrierShape
...
public class TableModelAdapter extends AbstractTableModel implements ShapeModelListener {
	/*
	 *  Adaptee, a reference to a Shape or a CarrierShape object that is to be 
	...
	/**
	 * Creates a TableModelAdapter and sets Shape/CarrierShape that is to be 
	 ...
	 * attributes. If this TableModelAdapter is set with a CarrierShape, this
	...
	public int getRowCount() {
		...
		if(_adaptee instanceof CarrierShape) {
			CarrierShape nestingShape = (CarrierShape)_adaptee;
	...
	/**
	 * Sets the adaptee Shape/CarrierShape object that should be represented by 
	...
	public void update(ShapeModelEvent event) {
		...
		if(eventType == ShapeModelEvent.EventType.ShapeAdded) {
			CarrierShape parent = shape.parent();
			if(parent == _adaptee) {
